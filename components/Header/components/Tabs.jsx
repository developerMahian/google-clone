...
import { FaBookReader, FaPlayCircle, FaRegImages, FaRegNewspaper } from "react-icons/fa";
...
const TabBtn = ({ children, tabId }) => {
	...
	const tabActiveValue = router.query.tab === tabId || (router.asPath === "/" && tabId === "articles");
	...
	return (
		<button
			className={`flex items-center gap-2 text-[15px] sm:text-base border-b-2 ${
				isTabActive ? "border-sky-500 child-svg:text-sky-500 font-medium" : "border-transparent"
			} p-1`}
