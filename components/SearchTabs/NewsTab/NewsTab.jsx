...
const NewsTab = () => {
	...
	const { isLoading, data } = useQuery(
		["newsSearch", debouncedSearchTerm],
		() => getResults("searchApi", "news"),
		{
...
const NewsCollection = ({ entries }) => {
	...
	if (sortByTime) {
		displayItems = filteredEntries.slice(itemsVisited, itemsVisited + itemsPerPage);
	} else {
		displayItems = entries.slice(itemsVisited, itemsVisited + itemsPerPage);
	}
		...
		<div className="flex flex-col gap-3 max-w-3xl mb-6">
			{filterLoading ? (
				...
				displayItems?.map(({ id, link, published, source, title }) => (
					<div
						key={id}
						className="px-5 py-4 md:pr-12 rounded-lg border border-gray-300 dark:border-gray-700"
					>
						<a href={link} className="inline-block group" target="_blank" rel="noreferrer">
							...
							<h1 className="text-sky-600 dark:text-sky-300 text-lg sm:text-xl group-hover:underline underline-offset-4 mb-1">
								{title}
							</h1>

							<span className="opacity-70 text-xs">
								{timeAgo.format(new Date(published), "round")}
							</span>
						</a>
					</div>
				))
			...
			<Pagination
				results={sortByTime ? filteredEntries : entries}
